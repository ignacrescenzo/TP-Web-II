...
class Model_Comercio extends Model
{
    ...
    public function enviarEmailDeConfirmacion($usuarioParaEmail)
    {
        ...
        $mensaje = "Para seguir la confirmacion de tu comercio entra a el siguiente link: <a href='localhost/login/cambiarContrasena?u=".$idUsuario."&c=".$idComercio."'> Click acá</a>";
    ...
    public function insertarComercio($nombreComercio, $email, $direccion, $telefono, $foto)
    {
        $conn =BaseDeDatos::conectarBD();
        if (file_exists("application/resources/upload/" . $foto["name"])) {
            unlink($_SERVER['DOCUMENT_ROOT']."/application/resources/upload/" . $foto["name"]);
        }
        move_uploaded_file(
                $foto["tmp_name"],
            "application/resources/upload/" . $foto["name"]
            );
        $sql = "insert into comercio (nombre,email,direccion,banner,telefono, habilitado,imagen) values ('".$nombreComercio."','".$email."','".$direccion."', null, '".$telefono."',0, '".$foto["name"]."' );";
        $result = mysqli_query($conn, $sql);
    }
